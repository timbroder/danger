...
RSpec.describe Danger::Scrutinizer do
  let(:valid_env) do
    {
      "SCRUTINIZER_PR_NUMBER" => "4",
      "SCRUTINIZER" => "true",
      "SCRUTINIZER_PROJECT" => "g/Kidfund/web"
    }
  end
  ...
  describe ".validates_as_pr?" do
    ...
    it "does not validate when there isn't a PR" do
      valid_env["SCRUTINIZER_PR_NUMBER"] = nil
  ...
  describe "#new" do
    it "sets the repo_slug" do
      expect(source.repo_slug).to eq("Kidfund/web")
    end
    ...
    it "sets the repo_url", host: :github do
      with_git_repo(origin: "git@github.com:Kidfund/web") do
        expect(source.repo_url).to eq("git@github.com:Kidfund/web")
      end
    end
  end
